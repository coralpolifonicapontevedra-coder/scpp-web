...
function preparar(indice, fotos, operacionId) {
  ...
  return {
    ...
    actualizadoDesde: `FOTOS-ADMIN-V2-FAST-${operacionId}`,
    version: '9'
  };
}
...
export async function onRequestFotosAdministracionV2Fast({ request, env }) {
  ...
  try {
    ...
    if (!edicion) {
      const copias = [];
      if (publicarPrivada && rutaPrivada) copias.push(copiarSeFalta(env.R2_PRIVADO, env.R2_PUBLICO, rutaPrivada));
      if (publicarPublica && rutaPublica) copias.push(copiarSeFalta(env.R2_PUBLICO, env.R2_PRIVADO, rutaPublica));
      if (publicarPrivada && miniPrivada) copias.push(copiarSeFalta(env.R2_PRIVADO, env.R2_PUBLICO, miniPrivada).catch(() => {}));
      if (publicarPublica && miniPublica) copias.push(copiarSeFalta(env.R2_PUBLICO, env.R2_PRIVADO, miniPublica).catch(() => {}));
    ...
    // Non se relén os catro índices: un put de R2 xa confirmou cada escritura.
    // A Sheet devolve tamén o estado lido despois de flush(), polo que evitamos
    // unha segunda rolda completa de verificación que era a principal fonte de espera.
    return json(200, {
      ok: true,
      idFoto: id,
      ...metadatos,
      ...
      backend: 'FOTOS-ADMIN-V2-FAST',
      permiso: 'CENTRAL+CACHE',
      verificacion: 'R2-PUT+SHEET-FLUSH',
    ...
    console.error('Erro no gardado rápido de Fotografías:', error);
    return json(503, {
      ok: false,
