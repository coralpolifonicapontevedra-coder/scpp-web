...
const json = (status, body) => new Response(JSON.stringify(body), {
  ...
  headers: {
    ...
    'X-SCPP-Delete-Version': 'FOTOS-ADMIN-DELETE-V3-PRODUCTION',
...
function preparar(indice, fotos, operacionId) {
  ...
  return {
    ...
    actualizadoDesde: `FOTOS-DELETE-PRODUCTION-V3-${operacionId}`,
    version: '9'
  };
}
...
async function comprobarR2Production(env, fotosObxectivo, id) {
  ...
  for (const clave of rutas) {
    ...
    if (fotoMarcadaPreview(privado) || fotoMarcadaPreview(publico)) {
      const erro = new Error('Borrado bloqueado: detectouse un obxecto R2 marcado como pertencente a Preview.');
      erro.codigo = 'R2_PREVIEW_OBJECT';
      throw erro;
    }
  }
...
async function chamarBorradoSheet(env, usuario, id, huerfana) {
  ...
  if (!resultado?.ok) {
    const erro = new Error(resultado?.erro || 'Non se puido eliminar a fotografía na Sheet de Producción.');
    erro.codigo = resultado?.codigo || '';
    throw erro;
  }
  if (resultado.entorno !== 'production') throw new Error('Apps Script non confirmou o entorno de Producción.');
...
export async function onRequestFotosDeleteV3({ request, env }) {
  ...
  if (!PRODUCTION_HOSTS.has(host)) {
    return json(403, { ok: false, erro: 'O borrado de Producción non está habilitado neste dominio.' });
  }

  if (!env.FIREBASE_API_KEY || !env.WEB_WRITE_TOKEN || !env.R2_PUBLICO || !env.R2_PRIVADO) {
    return json(500, { ok: false, erro: 'O borrado de fotografías de Producción non está configurado.' });
  }
  ...
  try {
    ...
    if (!obxectivo.length) return json(404, { ok: false, erro: 'Fotografía non localizada nos índices R2 de Producción.' });

    const comprobacion = await comprobarR2Production(env, obxectivo, id);
    ...
    const limpeza = { publicos: 0, privados: 0, prefixos: 0, huerfana, aviso: '' };
    ...
    if (huerfana) {
      ...
      try {
        ...
        for (const prefix of [`fotos/editadas/${id}-`, `fotos/editadas-miniaturas/${id}-`]) {
        ...
        limpeza.aviso = 'A fotografía foi eliminada dos índices, Sheet e Drive de Producción, pero pode quedar algún ficheiro R2 orfo.';
      }
    }

    return json(200, {
      ok: true,
      backend: 'FOTOS-ADMIN-DELETE-V3-PRODUCTION',
      entorno: 'production',
      ...
      mensaxe: limpeza.aviso || sheet.mensaxe || sheet.avisoDrive || (
        huerfana
          ? 'Rexistro fotográfico huérfano eliminado e verificado en Producción.'
          : 'Fotografía eliminada e verificada en Producción.'
      )
    });
  } catch (erro) {
    if (indicesActualizados && backup) await rollbackIndices(env, backup);
    console.error('Erro no borrado de fotografía en Producción:', erro);
