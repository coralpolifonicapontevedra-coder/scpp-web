...
const json = (status, body) => new Response(JSON.stringify(body), {
  ...
  headers: {
    ...
    'X-SCPP-Delete-Version': 'FOTOS-ADMIN-DELETE-V4',
...
export async function onRequestFotosDeleteV4(context) {
  ...
  if (new URL(request.url).hostname.toLowerCase() !== PREVIEW_HOST) return primeira;
  ...
  try {
    ...
    if (!residuos.length) {
      return json(409, {
        ok: false,
        backend: 'FOTOS-ADMIN-DELETE-V4',
        codigo: 'R2_RECOVERY_EMPTY',
        erro: 'O estado R2 cambiou durante a comprobación. Non se realizou ningún borrado.'
      });
    }

    const nonVerificados = residuos.filter((item) => !fotoMarcadaPreview(item.head));
    if (nonVerificados.length) {
      return json(409, {
        ok: false,
        backend: 'FOTOS-ADMIN-DELETE-V4',
        codigo: 'R2_RECOVERY_NOT_VERIFIED',
        erro: 'Borrado bloqueado: hai obxectos asociados ao identificador que non se puideron verificar como propios de Preview.'
      });
    }
    ...
    console.error('Erro recuperando rutas R2 para o borrado v4:', erro);
    return json(409, {
      ok: false,
      backend: 'FOTOS-ADMIN-DELETE-V4',
