...
const json = (status, body) => new Response(JSON.stringify(body), {
  ...
  headers: {
    ...
    'X-SCPP-Delete-Version': 'FOTOS-ADMIN-DELETE-V5-FAST',
...
function preparar(indice, fotos, operacionId) {
  ...
  return {
    ...
    actualizadoDesde: `FOTOS-DELETE-V5-FAST-${operacionId}`,
    version: '10'
  };
}
...
async function comprobarRutasProduction(env, rutas) {
  const claves = [...rutas];
  ...
  if (comprobacions.some(fotoMarcadaPreview)) {
...
async function chamarBorradoSheet(env, usuario, id) {
  ...
  if (resultado.entorno !== 'production') {
    throw new Error('Apps Script non confirmou o entorno de Producción.');
  }
  return resultado;
}
...
export async function onRequestFotosDeleteV5Fast(context) {
  ...
  // A autorización destrutiva real volve comprobarse no Apps Script de Producción.
  // Esta caché evita unha segunda chamada previa a Apps Script que provocaba 504.
  ...
  try {
    ...
    if (!rutasObxectivo.size) {
      return onRequestFotosDeleteV4(context);
    }
    await comprobarRutasProduction(env, rutasObxectivo);
    ...
    return json(200, {
      ok: true,
      idFoto: id,
      backend: 'FOTOS-ADMIN-DELETE-V5-FAST',
      entorno: 'production',
      sheet,
      limpezaR2: 'segundo-plano',
      tempoRespostaMs: Date.now() - inicio,
      mensaxe: sheet?.avisoDrive || 'Fotografía eliminada definitivamente de Producción.'
    });
  } catch (error) {
