...
async function gardarSheet(env, usuario, datos) {
  ...
  if (!resultado?.ok) {
    throw new Error(resultado?.erro || 'Non se puido gardar a fotografía na Sheet');
  }
  if (
...
async function ler(bucket, clave) {
  ...
  if (!datos || !Array.isArray(datos.fotos)) {
    throw new Error(`Índice R2 non válido: ${clave}`);
  }
  return datos;
}
...
function preparar(indice, fotos, operacionId) {
  ...
  return {
    ...
    actualizadoDesde: `FOTOS-ADMIN-V2-${operacionId}`,
    version: '8'
  };
}
...
async function copiarSeFalta(bucketDestino, bucketOrixe, clave) {
...
async function prepararEdicion(env, usuario, id, datos, publicarPublica) {
  ...
  if (publicarPublica) {
    await escribirImaxe(env.R2_PUBLICO, rutaPublica, bytes, mimeType, usuario, id, 'edicion', true);
  }
  if (thumbBytes) {
...
export async function onRequestFotosAdministracionV2({ request, env }) {
  if (request.method !== 'POST') return json(405, { ok: false, erro: 'Método non permitido' });
  if (!env.FIREBASE_API_KEY || !env.WEB_WRITE_TOKEN || !env.R2_PUBLICO || !env.R2_PRIVADO) {
    return json(500, { ok: false, erro: 'Administración de fotografías v2 non configurada' });
  }
  ...
  if (texto(datos.accion || 'gardar') !== 'gardar') {
    return json(400, { ok: false, erro: 'Acción non permitida no backend v2' });
  }
  ...
  try {
    ...
    let rutaPrivada = edicion?.rutaPrivada || ruta(base, 'privada') || ruta(base, 'publica');
    let rutaPublica = edicion?.rutaPublica || ruta(base, 'publica') || rutaPrivada;
    let miniPrivada = edicion?.rutaMiniaturaPrivada || miniatura(base, 'privada') || miniatura(base, 'publica');
    let miniPublica = edicion?.rutaMiniaturaPublica || miniatura(base, 'publica') || miniPrivada;

    if (!rutaPrivada && !rutaPublica) throw new Error('A fotografía non ten unha ruta R2 recuperable');

    if (!edicion) {
      if (publicarPrivada && rutaPrivada) {
        await copiarSeFalta(env.R2_PRIVADO, env.R2_PUBLICO, rutaPrivada);
      }
      if (publicarPublica && rutaPublica) {
        await copiarSeFalta(env.R2_PUBLICO, env.R2_PRIVADO, rutaPublica);
      }
      if (publicarPrivada && miniPrivada) {
        await copiarSeFalta(env.R2_PRIVADO, env.R2_PUBLICO, miniPrivada).catch(() => {});
      }
      if (publicarPublica && miniPublica) {
        await copiarSeFalta(env.R2_PUBLICO, env.R2_PRIVADO, miniPublica).catch(() => {});
      }
    }
    ...
    const [pubV, priV, revV, catV] = await Promise.all([
      ler(env.R2_PUBLICO, INDEX_PUBLICO),
      ler(env.R2_PRIVADO, INDEX_PRIVADO),
      ler(env.R2_PRIVADO, INDEX_REVISION),
      ler(env.R2_PRIVADO, CATALOGO)
    ]);
    const enPublica = pubV.fotos.some((foto) => idFoto(foto) === id);
    const enPrivada = priV.fotos.some((foto) => idFoto(foto) === id);
    const enRevision = revV.fotos.some((foto) => idFoto(foto) === id);
    const catalogada = catV.fotos.find((foto) => idFoto(foto) === id);

    if (enPublica !== publicarPublica || enPrivada !== publicarPrivada) {
      throw new Error('A verificación final de R2 non coincide co estado solicitado');
    }
    if (enRevision) throw new Error('A fotografía segue figurando como pendente en R2');
    if (!catalogada || texto(catalogada.titulo) !== metadatos.titulo) {
      throw new Error('O catálogo R2 non confirmou os metadatos gardados');
    }
    ...
    return json(200, {
      ...
      backend: 'FOTOS-ADMIN-V2',
      permiso: 'CENTRAL',
      mensaxe: edicion
        ? 'Imaxe, datos, Sheet e R2 gardados e verificados polo módulo novo.'
        : 'Datos, publicación, Sheet e R2 gardados e verificados polo módulo novo.'
    });
  } catch (error) {
    console.error('Erro no backend novo de Fotografías:', error);
    return json(503, {
      ok: false,
