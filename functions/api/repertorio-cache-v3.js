...
const CATALOG_VERSION = 'repertorio-cache-v3-swr';
...
function catalogoValido(catalogo) {
  return catalogo?.ok === true && Array.isArray(catalogo?.obras);
}

function snapshotAdminValido(cache) {
...
function mapAudio(row) {
  const rawWorkId = canonId(row.NomeObra);
...
function construirCatalogo(snapshot, concertIndex, catalogoAnterior) {
  ...
  for (const row of audios) {
    ...
    const sha = clean(row.R2SHA256);
    if (sha) {
      const fingerprint = [workId, clean(row.Voz).toLowerCase(), clean(row.TipoAudio).toLowerCase(), sha].join('|');
...
async function gardarCatalogo(env, snapshot, savedAt, concertIndex, catalogoAnterior, source) {
  const catalogo = construirCatalogo(snapshot, concertIndex, catalogoAnterior);
  catalogo.cacheMeta = {
    savedAt,
    source,
  ...
  await writeJson(env.R2_PRIVADO, catalogoKey(env), catalogo);
  return catalogo;
}

async function refrescarCatalogoDesdeSheets(env, user, catalogoAnterior) {
  ...
  return gardarCatalogo(env, payload, savedAt, concertIndex, catalogoAnterior, 'Sheets→R2');
}

function programarRefresco(waitUntil, env, user, catalogoAnterior) {
  const tarefa = refrescarCatalogoDesdeSheets(env, user, catalogoAnterior)
    .catch((error) => console.error('Non se puido refrescar Repertorio en segundo plano:', error));
...
export async function onRequest({ request, env, waitUntil }) {
  ...
  const [catalogoAnterior, adminCache] = await Promise.all([
    readJson(env.R2_PRIVADO, catalogoKey(env)),
    readJson(env.R2_PRIVADO, adminKey(env))
  ]);

  const catalogSavedAt = Number(catalogoAnterior?.cacheMeta?.savedAt || 0);
  const adminSavedAt = Number(adminCache?.gardadoEn || 0);

  if (snapshotAdminValido(adminCache) && (!catalogoValido(catalogoAnterior) || adminSavedAt > catalogSavedAt)) {
    const concertIndex = await readJson(env.R2_PRIVADO, concertosKey(env));
    const catalogo = await gardarCatalogo(
      env,
      adminCache.payload,
      adminSavedAt || Date.now(),
      concertIndex,
      catalogoAnterior,
      'R2-ADMIN-SNAPSHOT'
    );

    if (Date.now() - adminSavedAt > CACHE_FRESH_MS) {
      programarRefresco(waitUntil, env, user, catalogo);
    }
  ...
  if (catalogoValido(catalogoAnterior)) {
    const idade = idadeCatalogo(catalogoAnterior);
    if (idade > CACHE_FRESH_MS) programarRefresco(waitUntil, env, user, catalogoAnterior);

    return json(200, catalogoAnterior, {
      'X-SCPP-Repertorio': idade <= CACHE_FRESH_MS ? 'R2-CACHE' : 'R2-STALE-WHILE-REVALIDATE',
      'X-SCPP-Repertorio-Source': clean(catalogoAnterior?.cacheMeta?.source) || 'R2',
      'X-SCPP-Repertorio-Age': String(Math.round(idade / 1000)),
      'Server-Timing': `r2;dur=${Date.now() - inicio}`
    });
  }

  try {
    const catalogo = await refrescarCatalogoDesdeSheets(env, user, null);
    return json(200, catalogo, {
      'X-SCPP-Repertorio': 'R2-COLD-REFRESH',
      'X-SCPP-Repertorio-Source': 'SHEETS-SNAPSHOT',
      'Server-Timing': `sync;dur=${Date.now() - inicio}`
    });
  } catch (error) {
    console.error('Non se puido crear o catálogo inicial de Repertorio:', error);
    return json(503, {
      ok: false,
