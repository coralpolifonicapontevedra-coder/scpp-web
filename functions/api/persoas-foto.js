...
async function handleGet(context) {
  ...
  if (url.searchParams.get('download') !== '1') {
    return json(200, {
      ok: true,
      disponible: true,
      mimeType: String(index.mimeType || ''),
      actualizadaEn: String(index.actualizadaEn || '')
    });
  }
  ...
  return new Response(object.body, {
    status: 200,
    headers: {
      'Content-Type': String(index.mimeType || object.httpMetadata?.contentType || 'application/octet-stream'),
      'Cache-Control': 'private, no-store',
...
async function handlePost(context) {
  ...
  if (!contentType.toLowerCase().startsWith('multipart/form-data')) {
    return json(415, { ok: false, erro: 'Formato de subida non admitido.' });
  }
  ...
  if (!context.env.R2_PRIVADO || typeof context.env.R2_PRIVADO.put !== 'function') {
    return json(503, { ok: false, erro: 'O almacenamento privado non está dispoñible.' });
  }
  ...
  await context.env.R2_PRIVADO.put(key, buffer, {
    httpMetadata: {
    ...
    customMetadata: {
      idPersoa,
      revisionId: safeId(invitation.revisionId || ''),
      actualizadaEn: now
    }
  });

  await context.env.R2_PRIVADO.put(photoIndexKey(idPersoa), JSON.stringify({
    key,
    mimeType,
    size: file.size,
    actualizadaEn: now,
    revisionId: String(invitation.revisionId || '')
  }), {
    httpMetadata: {
