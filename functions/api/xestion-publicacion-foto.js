...
function fichaEstado(f,pub,pri){
  const id=idFoto(f),publica=pub.has(id),privada=pri.has(id),rev=estadoRevision(f);
  ...
  else if(rev.includes('rexe')||rev.includes('rechaz'))estadoXestion='rexeitada';
...
export async function onRequest({request,env}){
  ...
  try{
    ...
    if(accion==='listar'){
      ...
      const conta=tipo=>fotos.filter(f=>f.estadoXestion===tipo).length;
      return json(200,{ok:true,fotos,total:fotos.length,publicas:idsPub.size,privadas:idsPri.size,publicadas:conta('publicada'),nonPublicadas:conta('nonpublicada'),pendentes:conta('pendente'),rexeitadas:conta('rexeitada')});
    }
