...
async function getDraft(env,user,id){const key=draftKey(env,id),saved=await readJson(env.R2_PRIVADO,key);if(validDraft(saved,id))return saved;const result=await chamarAppsScript(env,user,'obterXestionConcertoAdministracionPortal',{idConcerto:id});return writeJson(env.R2_PRIVADO,key,fromManagement(result,id),'borrador-concerto');}
