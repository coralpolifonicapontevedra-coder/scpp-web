...
async function verificarAdmin(env,user){if(!env.R2_PRIVADO?.get)return false;const o=await env.R2_PRIVADO.get(`${ADMIN_CACHE_PREFIX}${await hashEmail(user.email)}.json`);if(!o)return false;const e=await o.json().catch(()=>null);return e?.administrador===user.email&&e?.payload?.perfil?.nivel==='Administración';}
