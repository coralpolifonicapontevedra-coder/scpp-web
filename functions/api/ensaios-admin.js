...
function podeAdministrar(payload) {
  return payload?.perfil?.podeEditar === true;
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (accion === 'cambiarData') {
      ...
      if (!id || !/^\d{4}-\d{2}-\d{2}$/.test(data)) {
        return erro(400, 'REQUEST', 'INVALID_DATA', 'Indica un ensaio e unha data válida.');
      }
      const result = await chamarAppsScript(env, user, 'actualizarEnsaioAdministracionPortal', {
        idEnsaio:id,
        data,
        cancelado:false
      });
