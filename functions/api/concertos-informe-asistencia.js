...
const reportKey = (env) => clean(env.CF_PAGES_BRANCH) === 'main' ? REPORT_MAIN : REPORT_PREVIEW;
...
export async function onRequest({ request, env }) {
  ...
  const object = await env.R2_PRIVADO.get(reportKey(env));
  ...
  if (!informe?.ok || !Array.isArray(informe?.informe?.niveis)) return json(502, { ok:false, erro:'O informe gardado non ten un formato válido.' });
