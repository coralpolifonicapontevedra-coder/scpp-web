...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (!resultado.ok) {
      const status = resultado.erro === 'Usuario non autorizado' ? 403 : 400;
      return json(status, resultado);
    }
    if (accion === 'obterFicheiroDocumentacion') return respostaFicheiro(resultado);
    return json(200, resultado);
  } catch (erro) {
    console.error(erro);
    return json(502, { ok: false, erro: 'Non foi posible contactar co servizo de documentación' });
  }
}
