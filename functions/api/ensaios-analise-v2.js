...
function analizar(base, desde, ata) {
  ...
  const persoas = (Array.isArray(base.persoas) ? base.persoas : []).map((p) => ({
  ...
  for (const a of asistencias) {
    ...
    if (!row) continue;
    const estado = clean(a.estadoAsistencia).toLowerCase();
  ...
  const porVoz = ['Soprano', 'Contralto', 'Tenor', 'Baixo'].map((voz) => {
    const rows = persoasStats.filter((p) => p.voz.toLowerCase() === voz.toLowerCase());
    const asiste = rows.reduce((sum, p) => sum + p.asiste, 0);
    const rexistrados = rows.reduce((sum, p) => sum + p.rexistrados, 0);
    return { voz, persoas: rows.length, asiste, rexistrados, porcentaxe: rexistrados ? Math.round(asiste * 100 / rexistrados) : 0 };
  });

  const porEnsaio = ensaios.map((e) => {
    const id = clean(e.idEnsaio);
    const rows = asistencias.filter((a) => clean(a.ensaio) === id && ['asiste', 'non asiste'].includes(clean(a.estadoAsistencia).toLowerCase()));
    const presentes = rows.filter((a) => clean(a.estadoAsistencia).toLowerCase() === 'asiste').length;
    return {
      idEnsaio: id,
  ...
  return {
    ...
    maiores,
    menores,
    persoas: pessoasOrdenadas(persoasStats),
    porEnsaio,
    porMes
  };
}

function pessoasOrdenadas(rows) {
  return [...rows].sort((a, b) => a.nome.localeCompare(b.nome, 'gl'));
}
...
export async function onRequest({ request, env }) {
  ...
  const desde = /^\d{4}-\d{2}-\d{2}$/.test(clean(body.desde)) ? clean(body.desde) : '';
  const ata = /^\d{4}-\d{2}-\d{2}$/.test(clean(body.ata)) ? clean(body.ata) : '';
  if (desde && ata && desde > ata) return json(400, { ok: false, erro: 'A data inicial non pode ser posterior á data final.' });

  try {
    const { payload, fonte } = await obterBase(env, user);
    return json(200, { ok: true, desde, ata, fonte, ...analizar(payload, desde, ata) });
  } catch (error) {
