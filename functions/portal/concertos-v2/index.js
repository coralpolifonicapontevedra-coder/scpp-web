const RUTA_IMPLEMENTACION = '/portal/concertos-v2-base/';
...
export async function onRequestGet({ request, env }) {
  ...
  if (branch !== 'main' && !html.includes('/api/asistencias-concertos-preview')) {
    if (html.includes('</head>')) html = html.replace('</head>', `${scriptAsistenciasPreview}</head>`);
    else html = `${scriptAsistenciasPreview}${html}`;
  }
  ...
  recursos.forEach((recurso) => {
    ...
    if (recurso.includes('stylesheet') && html.includes('</head>')) html = html.replace('</head>', `${recurso}</head>`);
    else if (html.includes('</body>')) html = html.replace('</body>', `${recurso}</body>`);
    else html += recurso;
  });
  ...
  cabeceiras.set('X-SCPP-Concertos-Version', 'v2-main-base');
...
export async function onRequest({ request, env }) {
  if (request.method === 'GET' || request.method === 'HEAD') return onRequestGet({ request, env });
