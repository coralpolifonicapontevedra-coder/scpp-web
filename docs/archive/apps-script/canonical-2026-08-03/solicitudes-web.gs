...
function enviarAvisoSolicitude_(contexto, valores) {
  ...
  var html =
    ...
    escaparHtmlSolicitude_(valores.Telefono || 'Non indicado') +
    '<br><strong>Entidade:</strong> ' +
    escaparHtmlSolicitude_(valores.Entidade || 'Non indicada') +
    '</p><p><strong>Mensaxe:</strong><br>' +
    escaparHtmlSolicitude_(valores.Mensaxe).replace(/\n/g, '<br>') +
    '</p><p><a href="' + enlace + '">Abrir SolicitudesWeb</a></p>' +
...
function actualizarCampoSolicitude_(folla, cabeceiras, fila, nomeCampo, valor) {
  ...
  if (indice !== -1) {
    folla.getRange(fila, indice + 1).setValue(valor);
  }
}
...
function crearIdSolicitude_(data, rowId) {
  ...
  return (
    'SOL-' + marca + '-' +
    String(rowId).replace(/-/g, '').slice(0, 8).toUpperCase()
  );
}
