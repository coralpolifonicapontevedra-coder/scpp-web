...
function sincronizarMediosConcertos() {
  ...
  CONFIG_MEDIOS.forEach((config) => {
    ...
    while (files.hasNext()) {
      ...
      const resultado = subirOuActualizarGitHub_(
        blob,
        nomeDestino,
        ruta,
...
function subirOuActualizarGitHub_(blob, nomeFicheiro, ruta, github) {
  ...
  if (codigo !== 200 && codigo !== 201) {
    throw new Error(
      `GitHub devolveu ${codigo} para ${ruta}: ${resposta.getContentText()}`
    );
  }
...
function obterFicheiroGitHub_(api, github) {
  const resposta = UrlFetchApp.fetch(
    `${api}?ref=${encodeURIComponent(github.branch)}`,
    {
      method: 'get',
      headers: {
        Authorization: `Bearer ${github.token}`,
  ...
  if (codigo !== 200) {
    throw new Error(
      `Non se puido consultar GitHub (${codigo}): ${resposta.getContentText()}`
    );
  }
...
function probarAccesoCarpetaConcertos() {
  const folderId = PropertiesService
    .getScriptProperties()
    .getProperty('CONCERTOS_IMAGES_FOLDER_ID');
