...
var ACCIONS_TPV_DOAZONS_ = [
  'crearDoazonTPVPortal',
  'actualizarDoazonTPVPortal'
];
...
function despacharFotosAdministracionPortal_(accion, datos, bloqueo) {
  ...
  if (accion === 'comprobarFotosAdministracionPortal') return comprobarFotosAdministracionPortal_(datos);
  if (accion === 'gardarFotoAdministracionPortal') {
    if (bloqueo && !bloqueo.hasLock()) {
      if (!bloqueo.tryLock(1500)) {
        return { ok: false, codigo: 'BUSY', erro: 'Hai outro gardado de Fotografías en curso. Téntao de novo nuns segundos.' };
      }
    }
...
function despacharTPVDoazons_(accion, datos, bloqueo) {
  if (ACCIONS_TPV_DOAZONS_.indexOf(accion) < 0) return null;
  if (bloqueo && !bloqueo.hasLock()) bloqueo.waitLock(10000);
  if (accion === 'crearDoazonTPVPortal') return crearDoazonTPVPortal_(datos);
  if (accion === 'actualizarDoazonTPVPortal') return actualizarDoazonTPVPortal_(datos);
  return null;
}
...
function despacharXestionPermisosPortal_(accion, datos, bloqueo) {
  ...
  if (respostaPersoas !== null) {
    rexistrarAcceso({
      ...
      tipoEvento: accion.indexOf('persoasV2') === 0 ? 'Persoas V2 · ' + accion : 'Administración · Persoas',
      modulo: 'Administración · Persoas',
      resultado: respostaPersoas.ok ? 'Correcto' : 'Rexeitado',
      detalle: respostaPersoas.ok
        ? String(respostaPersoas.idPersoa || respostaPersoas.enviados || respostaPersoas.total || respostaPersoas.version || respostaPersoas.cotaRestante || '')
  ...
  // As accións TPV só poden chegar desde Cloudflare despois de validar o
  // WEB_WRITE_TOKEN no doPost principal. Non requiren sesión de administrador.
  var respostaTPV = despacharTPVDoazons_(accion, datos, bloqueo);
  if (respostaTPV !== null) return respostaTPV;
  ...
  if (accion === 'listarPermisosPortal') {
    try { xestionPermisosInstalarTriggers_(); }
    catch (erroTriggerPermisos) {
      console.warn('Permisos: non se puideron verificar os triggers de caché: ' + String(erroTriggerPermisos && erroTriggerPermisos.message ? erroTriggerPermisos.message : erroTriggerPermisos));
    }
