...
function configurarPerfilPortal() {
  PropertiesService.getScriptProperties().setProperties({
    PERFIL_PERSOAS_SPREADSHEET_ID: '1XWgPYg4z410225Qu17REOiXQlb14Wit7GwoWCjlo9rQ',
    PERFIL_PERSOAS_SHEET_ID: '388888827',
    PERFIL_USUARIOS_SPREADSHEET_ID: '1anry8OEiJ5EuZ-LZtz0QM_13uHj3wn2KXnamXs7f8KI',
...
function obterPerfilPortal_(datos) {
  ...
  if (!persoa) {
    return {
      ok: false,
      erro: 'Non se atopou a ficha persoal asociada'
    };
  }
...
function actualizarPerfilPortal_(datos) {
  ...
  var correoContacto = limparPerfil_(
  ...
  if (
    ...
    return {
      ok: false,
      erro: 'O correo electrónico de contacto non é válido'
    };
  }
  ...
  if (cp && !/^\d{5}$/.test(cp)) {
    return {
      ok: false,
      erro: 'O código postal debe ter cinco cifras'
    };
  }

  var preferencia = limparPerfil_(
    datos.preferenciaComunicacion,
    60
  );

  var preferenciasPermitidas = [
    '',
    'Correo electrónico',
    'WhatsApp',
    'Teléfono'
  ...
  if (preferenciasPermitidas.indexOf(preferencia) === -1) {
    return {
      ok: false,
      erro: 'A canle de comunicación non é válida'
    };
  }

  var consentimento = limparPerfil_(
    datos.consentimentoFoto,
    80
  );
  ...
  if (consentimentosPermitidos.indexOf(consentimento) === -1) {
    return {
      ok: false,
      erro: 'A opción de autorización de imaxe non é válida'
    };
  }

  var dataNacemento = analizarDataPerfil_(
    datos.dataNacemento
  );

  if (dataNacemento.erro) {
    return {
      ok: false,
      erro: dataNacemento.erro
    };
  }
  ...
  var valores = {
    ...
    'ContactoEmerxencia': limparPerfil_(
      datos.contactoEmerxencia,
      180
    ),
    'TelefonoEmerxencia': limparPerfil_(
      datos.telefonoEmerxencia,
      40
    ),
    'PreferenciaComunicacion': preferencia,
    'ConsentimentoFoto': consentimento,
    'MostrarAniversario': valorBooleanoPerfil_(
      datos.mostrarAniversario
    ),
  ...
  if (novaFoto.ruta) {
    valores.FotoPerfil = novaFoto.ruta;
  }
  ...
  var persoaActualizada = obterFilaPersoaPerfil_(
    contexto,
    usuario,
    email
  );
  ...
  return {
    ...
    perfil: construírRespostaPerfil_(
      contexto,
      persoaActualizada
    )
  };
...
function obterContextoPerfil_() {
  ...
  var folderId = propiedades.getProperty(
  ...
  if (
    ...
    throw new Error(
      'Falta configurar o módulo Perfil'
    );
  }
  ...
  if (
    !follaPersoas ||
    follaPersoas.getName() !== 'Persoas'
  ) {
    throw new Error(
      'Non se atopou a folla Persoas configurada'
    );
  }

  if (
    !follaUsuarios ||
    follaUsuarios.getName() !== 'UsuariosWeb'
  ) {
    throw new Error(
      'Non se atopou a folla UsuariosWeb configurada'
    );
  }
...
function obterUsuarioActivoPerfil_(contexto, email) {
  var valores =
    contexto.follaUsuarios.getDataRange().getValues();
  ...
  for (var i = 1; i < valores.length; i += 1) {
    ...
    var correoFila = String(
      fila[indices.Email] || ''
    ).trim().toLowerCase();

    var activo = valorBooleanoPerfil_(
      fila[indices.Activo]
    );

    if (correoFila === email && activo) {
      return {
        referenciaPersoa: String(
          fila[indices.Persoa] || ''
        ).trim(),
        email: email
      };
...
function obterFilaPersoaPerfil_(
  contexto,
  usuario,
  email
) {
  var valores =
    contexto.follaPersoas.getDataRange().getValues();
  ...
  for (var i = 1; i < valores.length; i += 1) {
    ...
    var correo = String(
      fila[indices['Correo electrónico']] || ''
    ).trim().toLowerCase();
...
function construírRespostaPerfil_(
  contexto,
  persoa
) {
  ...
  var foto = obterFotoPerfilBase64_(
    contexto,
    rutaFoto
  );
  ...
  return {
    ...
    cidade: texto('Cidade'),
    cp: texto('CP'),
    dataNacemento: formatarDataInputPerfil_(
      valor('DataNacemento')
    ),
    contactoEmerxencia: texto(
      'ContactoEmerxencia'
    ),
    telefonoEmerxencia: texto(
      'TelefonoEmerxencia'
    ),
    preferenciaComunicacion: texto(
      'PreferenciaComunicacion'
    ),
    consentimentoFoto: texto(
      'ConsentimentoFoto'
    ),
    mostrarAniversario: valorBooleanoPerfil_(
    ...
    dataActualizacionPerfil:
      formatarDataHoraPerfil_(
...
function gardarFotoPerfil_(
  ...
  if (
    ...
    return {
      erro:
        'O formato da fotografía de perfil non é compatible'
    };
  }
  ...
  try {
    ...
    return {
      erro:
        'Non foi posible ler a fotografía de perfil'
    };
  }
  ...
  if (bytes.length > 2 * 1024 * 1024) {
    return {
      erro:
        'A fotografía de perfil supera o máximo permitido'
    };
  }

  var rowId = String(
    persoa.fila[
      persoa.indices['Row ID']
    ] || 'persoa'
  )
  ...
  var indiceFoto =
    persoa.indices.FotoPerfil;

  if (indiceFoto !== undefined) {
    eliminarFotoAnteriorPerfil_(
      contexto,
      String(
        persoa.fila[indiceFoto] || ''
      ).trim()
    );
  }

  var ficheiro =
    contexto.carpetaFotos.createFile(
      Utilities.newBlob(
        bytes,
        tipo,
        nome
  ...
  var prefixo = String(
    contexto.propiedades.getProperty(
      'PERFIL_FOTOS_APPSHEET_PATH'
    ) || 'Fotos_Perfil/'
  ).replace(/\/+/g, '') + '/';
...
function eliminarFotoAnteriorPerfil_(
  contexto,
  ruta
) {
  var nome = String(ruta || '')
    .split('/')
    .pop();
  ...
  var ficheiros =
    contexto.carpetaFotos.getFilesByName(nome);
...
function obterFotoPerfilBase64_(
  contexto,
  ruta
) {
  var nome = String(ruta || '')
  ...
  if (!ficheiros.hasNext()) {
    return {
      dataUrl: '',
      aviso:
        'A fotografía rexistrada non se atopou na carpeta privada'
    };
  }
  ...
  if (bytes.length > 2 * 1024 * 1024) {
    return {
      dataUrl: '',
      aviso:
        'A fotografía é demasiado grande para a previsualización'
    };
  }
...
function analizarDataPerfil_(valor) {
  ...
  if (!partes) {
    return {
      erro:
        'A data de nacemento non é válida'
    };
  }
