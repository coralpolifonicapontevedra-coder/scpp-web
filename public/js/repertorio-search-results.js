(() => {
  ...
  function setupSearchResults() {
    ...
    style.textContent = `
      ...
      .work-search-results { display:grid; margin-top:.7rem; overflow:hidden; border:1px solid #cfc6c1; border-radius:4px; background:#fff; box-shadow:0 8px 22px rgba(46,34,29,.06); }
      .work-search-feedback { display:flex; justify-content:space-between; gap:1rem; align-items:center; padding:.62rem .82rem; border-bottom:1px solid #e7e2de; background:#faf8f6; color:#5f5853; font-family:var(--fuente-minimalista,Aptos,Calibri,system-ui,sans-serif); font-size:.74rem; }
      .work-search-feedback strong { color:var(--color-principal,#6a1b29); font-size:.79rem; }
      .work-search-feedback span { color:#7a726d; }
      .work-search-result { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:.25rem 1rem; align-items:center; width:100%; padding:.72rem .85rem; border:0; border-bottom:1px solid #e7e2de; background:#fff; color:#332f2c; font-family:var(--fuente-minimalista,Aptos,Calibri,system-ui,sans-serif); text-align:left; cursor:pointer; }
      ...
      .work-search-open { grid-column:2; grid-row:1 / span 2; align-self:center; color:var(--color-principal,#6a1b29); font-size:.7rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; white-space:nowrap; }
      @media(max-width:680px){.work-search-feedback{align-items:flex-start;flex-direction:column;gap:.15rem}.work-search-result{grid-template-columns:minmax(0,1fr)}.work-search-open{grid-column:1;grid-row:auto;margin-top:.18rem}}
    `;
    ...
    const render = () => {
      ...
      matches.forEach((option) => {
        ...
        const title = document.createElement('strong'); title.textContent = label.title; button.append(title);
        if (label.author) { const author = document.createElement('small'); author.textContent = label.author; button.append(author); }
        const open = document.createElement('span'); open.className = 'work-search-open'; open.textContent = 'Abrir ficha'; button.append(open);
    ...
    search.addEventListener('keydown',(event) => {
      ...
      if (event.key === 'Enter') { const first = results.querySelector('.work-search-result'); if (first instanceof HTMLButtonElement) { event.preventDefault(); first.click(); } }
    });
    results.addEventListener('click',(event) => {
      ...
      window.setTimeout(() => { select.value = workId; select.dispatchEvent(new Event('change',{bubbles:true})); results.hidden = true; },0);
    });
  }
        ...
        try {
          ...
          all.onsuccess = () => {
            ...
            for (const item of records) {
              ...
              token = String(value?.stsTokenManager?.accessToken || value?.stsTokenManager?.refreshToken && value?.stsTokenManager?.accessToken || '');
              if (token) break;
            }
            resolve(token);
          };
  ...
  function setupEnsaiosWorks() {
    ...
    const style = document.createElement('style');
    style.textContent = `
      #repertoire-panel .work-link,#repertoire-panel .work-link:visited,#repertoire-panel .work-link:hover,#repertoire-panel .work-link:focus{color:#24211f!important;text-decoration:none!important}
      #repertoire-panel .work-link{display:flex!important;flex-direction:column;gap:.5rem!important}
      #repertoire-panel .work-link strong{color:#24211f!important;line-height:1.25}
      #repertoire-panel .work-link small{display:block;margin:0;color:#6d6762!important;line-height:1.25}
      #repertoire-panel .work-type,#repertoire-panel .work-from,#repertoire-panel .work-to{display:none!important}
      #repertoire-panel .work-fields{display:grid!important;grid-template-columns:minmax(0,1fr) auto auto auto!important;align-items:center;gap:.55rem!important}
      ...
      @media(max-width:680px){#repertoire-panel .work-fields{grid-template-columns:1fr!important}#repertoire-panel .save-work,#repertoire-panel .remove-work{width:100%}}
    `;
    document.head.append(style);

    const inferDefaultRehearsal = () => {
      if (activeRehearsalId) return;
      const future = Array.from(document.querySelectorAll('#calendar-list [data-rehearsal]'));
      const candidate = future[future.length - 1];
      if (candidate instanceof HTMLElement) activeRehearsalId = candidate.dataset.rehearsal || '';
    };
    ...
    const rename = () => {
      ...
      if (summary?.textContent) summary.textContent = summary.textContent.replace(/^\s*(\d+)\s+traballadas?\s*$/i,'$1 obras');
      document.querySelectorAll('#tracking-kpis span').forEach((node) => { if (node.textContent?.trim() === 'Obras traballadas') node.textContent = 'Obras'; });
    };

    const enhanceRows = () => {
      rename();
      inferDefaultRehearsal();
    ...
    document.addEventListener('click',async (event) => {
      ...
      if (rehearsalButton instanceof HTMLElement && rehearsalButton.dataset.rehearsal) {
        activeRehearsalId = rehearsalButton.dataset.rehearsal;
      }
      ...
      inferDefaultRehearsal();
      if (!idRepertorio || !activeRehearsalId) { if (message) message.textContent = '⚠ Non foi posible identificar a obra ou o ensaio.'; return; }
      ...
      try {
        ...
        const response = await fetch('/api/ensaios-eliminar', {
          ...
          body:JSON.stringify({idToken,idEnsaio:activeRehearsalId,idRepertorio})
        });
