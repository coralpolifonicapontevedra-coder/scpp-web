 ...
 *
 * Deseñado para que o Portal traballe en memoria e faga unha única escritura
 * ao gardar, seguindo o patrón de Ensaios. As lecturas habituais deben chegar
 * desde R2; estas funcións úsanse para editar e persistir cambios nas Sheets.
 */
...
function nomePersoaConcertoOperacion_(row) {
  var completo = textoEnsaiosPortal_(campoEnsaiosPortal_(row, [
    'Nome_Completo', 'Nome completo', 'Nome e apelidos', 'NomeApelidos'
  ]));
  if (completo) return completo;
  return [
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Nome'])),
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Apelidos', 'Apellidos']))
...
function persoaActivaConcertoOperacion_(row) {
  ...
  if (activo !== '' && activo !== null && activo !== undefined) {
    return booleanoEnsaiosPortal_(activo);
  }
...
function listarConcertoOperacionPortal_(datos) {
  ...
  var concertoRow = concertos.find(function (row) {
    return textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Id', 'Id_Concerto', 'IdConcerto'])) === idConcerto;
  });
  ...
  var persoas = persoasRows.map(function (row) {
    return {
      ...
      nome: nomePersoaConcertoOperacion_(row),
      voz: textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Voz'])) || 'Sen voz indicada',
      activa: persoaActivaConcertoOperacion_(row)
    };
  ...
  persoas.sort(function (a, b) {
    ...
    return dv || a.nome.localeCompare(b.nome, 'gl', { sensitivity:'base' });
  });
  ...
  var asistentes = asistenciasRows.filter(function (row) {
    ...
    return textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Persoa', 'Id_Persoa', 'IdPersoa']));
  }).filter(Boolean);
  ...
  var catalogo = repertorioRows.map(function (row) {
    return {
      ...
      nome: textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['NomeObra', 'Nome', 'Obra'])),
      autor: textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Compositor', 'Autor']))
    };
  ...
  var programa = programaRows.filter(function (row) {
    return textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Id_Conciertos', 'Concerto', 'IdConcerto'])) === idConcerto;
  }).map(function (row) {
    return {
      idRelacion: textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Id', 'Row ID'])),
      idRepertorio: textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Id_Repertorio', 'Id_Obras', 'Obra'])),
      orde: Number(campoEnsaiosPortal_(row, ['Orde'])) || 999,
      notas: textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Notas'])),
  ...
  return {
    ...
    concerto:{
      ...
      data:serializarDataEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Data'])),
      nome:textoEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Nome'])),
      cidade:textoEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Cidade'])),
      lugar:textoEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Lugar'])),
      hora:serializarHoraEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Hora'])),
      estado:textoEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Estado'])),
      cartel:textoEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Cartel'])),
      triptico:textoEnsaiosPortal_(campoEnsaiosPortal_(concertoRow, ['Triptico', 'Tríptico']))
    },
    persoas:persoas,
...
function gardarAsistenciasConcertoPortal_(datos) {
  ...
  if (!idConcerto) return { ok:false, codigo:'VALIDATION', erro:'Falta o identificador do concerto' };
  if (idsPersoas.length > 200) return { ok:false, codigo:'VALIDATION', erro:'Hai demasiadas persoas na solicitude' };

  var cfg = configuracionConcertosOperacionPortal_();
  var persoasData = filasEnsaiosAdministracionPortal_(cfg.persoasId, 'Persoas', 'CONCERTOS_PERSOAS_SPREADSHEET_ID');
  var persoasMap = {};
  persoasData.rows.forEach(function (row) {
    var id = textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Id', 'Id_Persoa', 'Row ID']));
    if (id) persoasMap[id] = row;
  });
  var inexistentes = idsPersoas.filter(function (id) { return !persoasMap[id]; });
  if (inexistentes.length) return { ok:false, codigo:'VALIDATION', erro:'Hai persoas que xa non existen na táboa Persoas' };
  ...
  if (idxId < 0 || idxConcerto < 0 || idxPersoa < 0) {
    return { ok:false, codigo:'SCHEMA', erro:'A folla AsistenciasConcertos non ten as columnas esperadas' };
  }

  var lock = LockService.getScriptLock();
  try {
    lock.waitLock(10000);
    datosFolla.rows.filter(function (row) {
      return textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Concerto', 'Id_Conciertos', 'IdConcerto'])) === idConcerto;
    }).map(function (row) { return row.__row; }).sort(function (a, b) { return b - a; }).forEach(function (rowNumber) {
      datosFolla.sheet.deleteRow(rowNumber);
    });

    if (idsPersoas.length) {
      var filas = idsPersoas.map(function (idPersoa) {
        var rowPersoa = persoasMap[idPersoa];
        var fila = headers.map(function () { return ''; });
        fila[idxId] = Utilities.getUuid();
        fila[idxConcerto] = idConcerto;
        fila[idxPersoa] = idPersoa;
        if (idxVoz >= 0) fila[idxVoz] = textoEnsaiosPortal_(campoEnsaiosPortal_(rowPersoa, ['Voz']));
        if (idxEstado >= 0) fila[idxEstado] = true;
        return fila;
      });
...
function gardarProgramaConcertoPortal_(datos) {
  ...
  repertorio.forEach(function (row) {
    var id = textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Id', 'Id_Repertorio', 'Row ID']));
    if (id) idsValidos[id] = true;
  });

  var limpos = [];
  var usados = {};
  for (var i = 0; i < items.length; i += 1) {
    var idRepertorio = textoEnsaiosPortal_(items[i] && items[i].idRepertorio);
    if (!idRepertorio || usados[idRepertorio]) continue;
    if (!idsValidos[idRepertorio]) return { ok:false, codigo:'VALIDATION', erro:'Unha das obras xa non existe en Repertorio' };
    usados[idRepertorio] = true;
    limpos.push({
      idRepertorio:idRepertorio,
      orde:limpos.length + 1,
      notas:textoEnsaiosPortal_(items[i].notas).slice(0, 1000),
      solista:textoEnsaiosPortal_(items[i].solista).slice(0, 250)
    });
  }
  ...
  if (idxId < 0 || idxConcerto < 0 || idxRepertorio < 0 || idxOrde < 0) {
    return { ok:false, codigo:'SCHEMA', erro:'A folla ConcertosRepertorio non ten as columnas esperadas' };
  }
  ...
  try {
    ...
    if (limpos.length) {
      var filas = limpos.map(function (item) {
        var fila = headers.map(function () { return ''; });
        fila[idxId] = Utilities.getUuid();
        fila[idxOrde] = item.orde;
        fila[idxConcerto] = idConcerto;
        fila[idxRepertorio] = item.idRepertorio;
        if (idxNotas >= 0) fila[idxNotas] = item.notas;
        if (idxSolista >= 0) fila[idxSolista] = item.solista;
        return fila;
      });
      datosFolla.sheet.getRange(datosFolla.sheet.getLastRow() + 1, 1, filas.length, headers.length).setValues(filas);
    }
