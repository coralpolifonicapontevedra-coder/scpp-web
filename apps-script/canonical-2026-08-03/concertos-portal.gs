...
function obterDocumentoConcerto_(datos) {
  ...
  if (!email) {
    return { ok: false, erro: 'Falta o correo da persoa usuaria' };
  }

  if (!concertoId) {
    return { ok: false, erro: 'Falta identificar o concerto' };
  }
...
function usuarioActivoConcertosPortal_(contexto, email) {
  ...
  for (var i = 1; i < valores.length; i += 1) {
    ...
    var activo = valorBooleanoConcertosPortal_(
...
function valorBooleanoConcertosPortal_(valor) {
  ...
  return [
    'true', 'verdadero', 'verdadeiro', 'si', 'sí', 'yes', '1'
