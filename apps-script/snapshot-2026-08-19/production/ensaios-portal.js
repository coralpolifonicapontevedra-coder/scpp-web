...
function permisoEnsaiosPortal_(email) {
  ...
  var row = datos.rows.find(function(item) {
    return textoEnsaiosPortal_(
      campoEnsaiosPortal_(item, ['Email', 'Correo', 'CorreoElectronico'])
    ).toLowerCase() === correo;
  });
  ...
  if (
    activo !== '' &&
    ['baixa', 'baja', 'inactivo', 'inactiva', 'false', '0'].indexOf(
      textoEnsaiosPortal_(activo).toLowerCase()
    ) >= 0
  ) {
    return {
      autorizado: false,
  ...
  var cargo = normalizarEnsaiosPortal_(
    campoEnsaiosPortal_(row, ['Cargo'])
  );
  ...
  var cargosXunta = [
    ...
    'contador',
    'contadora',
    'arquiveirobibliotecario',
    'arquiveirabibliotecaria',
    'vogal',
    'vogais',
    'vocal',
    'vocales'
  ...
  var escritura = cargosXunta.some(function(item) {
    return cargo === item || cargo.indexOf(item) === 0;
  });

  var direccion =
    cargo.indexOf('director') === 0 ||
    cargo.indexOf('directora') === 0 ||
    cargo.indexOf('direccion') === 0;
