...
function obterXestionConcertoAdministracionPortal_(datos){
  ...
  var marcadas={};as.forEach(function(r){if(textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Concerto','Id_Conciertos']))===id&&booleanoEnsaiosPortal_(campoEnsaiosPortal_(r,['Estado asistencia','EstadoAsistencia'])))marcadas[textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Persoa']))]=true;});
  var persoas=ps.map(function(r){var pid=textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Id','Row ID']));return{id:pid,nome:textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Nome'])),primeiroApelido:textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Primeiro apelido','PrimeiroApelido'])),segundoApelido:textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Segundo apelido','SegundoApelido'])),voz:textoEnsaiosPortal_(campoEnsaiosPortal_(r,['Voz'])),asiste:!!marcadas[pid]};}).filter(function(p){return p.id&&p.nome&&p.voz;});
...
function gardarAsistentesConcertoAdministracionPortal_(datos){
  ...
  items.forEach(function(x){f.sheet.appendRow(f.headers.map(function(h){if(['Id_Asistencia','Id'].indexOf(h)>=0)return Utilities.getUuid();if(['Concerto','Id_Conciertos'].indexOf(h)>=0)return id;if(h==='Persoa')return textoEnsaiosPortal_(x.id);if(h==='Voz')return textoEnsaiosPortal_(x.voz);if(['Estado asistencia','EstadoAsistencia'].indexOf(h)>=0)return true;return '';}));});SpreadsheetApp.flush();return{ok:true,total:items.length};
}
