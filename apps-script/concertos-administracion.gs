...
function crearConcertoAdministracionPortal_(datos) {
  ...
  if (nome.length > 250 || cidade.length > 150 || lugar.length > 250 || caracteristicas.length > 3000) {
    return { ok:false, codigo:'VALIDATION', erro:'Algún dos textos supera a lonxitude permitida' };
  }
...
function eliminarConcertoAdministracionPortal_(datos) {
  ...
  var repertorio = filasEnsaiosAdministracionPortal_(cfg.concertosRepertorioId, 'ConcertosRepertorio', 'CONCERTOS_REPERTORIO_SPREADSHEET_ID').rows;
  var asistencias = filasEnsaiosAdministracionPortal_(cfg.asistenciasId, 'AsistenciasConcertos', 'ASISTENCIAS_CONCERTOS_SPREADSHEET_ID').rows;
  var obras = repertorio.filter(function (item) {
    return textoEnsaiosPortal_(campoEnsaiosPortal_(item, ['Id_Conciertos', 'Concerto', 'IdConcerto'])) === idConcerto;
  }).length;
  var persoas = asistencias.filter(function (item) {
    return textoEnsaiosPortal_(campoEnsaiosPortal_(item, ['Concerto', 'Id_Conciertos', 'IdConcerto'])) === idConcerto;
  }).length;
  var tenMedios = Boolean(
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Cartel'])) ||
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Triptico', 'Tríptico'])) ||
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['Prensa']))
  );
  var historico = Boolean(
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['NumeroConcerto'])) ||
    textoEnsaiosPortal_(campoEnsaiosPortal_(row, ['OrdeHistorica']))
  );

  if (obras || persoas || tenMedios || historico) {
    var motivos = [];
    if (obras) motivos.push(obras + ' obra' + (obras === 1 ? '' : 's') + ' no programa');
    if (persoas) motivos.push(persoas + ' asistencia' + (persoas === 1 ? '' : 's'));
    if (tenMedios) motivos.push('cartel, tríptico ou prensa asociados');
    if (historico) motivos.push('numeración histórica');
    return { ok:false, codigo:'HAS_RELATIONS', erro:'Non se pode dar de baixa este concerto porque ten ' + motivos.join(', ') + '. Retira primeiro esas relacións ou usa o estado Cancelado.' };
  }
  ...
  try {
    ...
    throw new Error('Diagnóstico CONCERTOS_SPREADSHEET_ID (' + cfg.concertosId + '): fallou a baixa. ' + String(erro && erro.message ? erro.message : erro));
  }
