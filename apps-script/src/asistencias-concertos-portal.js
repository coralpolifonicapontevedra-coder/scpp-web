...
function listarAsistenciasConcertosPortal_(datos) {
  ...
  Object.keys(porConcerto).forEach(
    function(idConcerto) {
      porConcerto[idConcerto].sort(
        function(a, b) {
          const diferenzaVoz =
            (ordeVoces[a.voz] || 99) -
            (ordeVoces[b.voz] || 99);
...
function lerFollaAsistenciasConcertos_() {
  ...
  if (!folla) {
    throw new Error(
      'Non se atopou a folla AsistenciasConcertos'
    );
  }
  ...
  const cabeceiras = valores[0].map(
    function(valor) {
