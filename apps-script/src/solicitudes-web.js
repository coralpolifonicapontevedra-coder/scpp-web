...
function rexistrarSolicitudeWeb_(datos) {
  ...
  if (datos.aceptacionProteccionDatos !== true) {
    return {
      ok: false,
      erro: 'Non consta a aceptación da protección de datos'
    };
  }
  ...
  var valores = {
    ...
    'FonteEntrada':
      limparSolicitude_(datos.fonteEntrada, 40) || 'Web pública',
    'Id_Interesado': '',
    'Id_Colaboracion': '',
    'ReferenciaTecnica':
      limparSolicitude_(datos.referenciaTecnica, 300)
  };
  ...
  try {
    ...
    avisoErro = String(
      (erroAviso && erroAviso.message) || erroAviso
    );

    console.error(
      'Non se puido enviar o aviso de SolicitudesWeb: ' + avisoErro
    );
  }
  ...
  return {
    ...
    aviso: avisoErro
      ? 'A solicitude gardouse, pero o aviso interno quedou pendente'
      : ''
  };
}
...
function enviarAvisoSolicitude_(contexto, valores) {
  ...
  var html =
    '<p>Recibiuse unha nova solicitude desde a web da SCPP.</p>' +
    '<p><strong>Referencia:</strong> ' +
    escaparHtmlSolicitude_(valores.Id_Solicitude) +
    '<br>' +
    '<strong>Orixe:</strong> ' +
    escaparHtmlSolicitude_(valores.Orixe) +
    '<br>' +
    '<strong>Tipo:</strong> ' +
    escaparHtmlSolicitude_(valores.TipoSolicitude) +
    '<br>' +
    '<strong>Nome:</strong> ' +
    escaparHtmlSolicitude_(valores.NomeCompleto) +
    '<br>' +
    '<strong>Correo:</strong> ' +
    escaparHtmlSolicitude_(valores.CorreoElectronico) +
    '<br>' +
    '<strong>Teléfono:</strong> ' +
    escaparHtmlSolicitude_(
      valores.Telefono || 'Non indicado'
    ) +
    '<br>' +
    '<strong>Entidade:</strong> ' +
    escaparHtmlSolicitude_(
      valores.Entidade || 'Non indicada'
    ) +
    '</p>' +
    '<p><strong>Mensaxe:</strong><br>' +
    escaparHtmlSolicitude_(valores.Mensaxe)
      .replace(/\n/g, '<br>') +
    '</p>' +
    '<p><a href="' +
    enlace +
    '">Abrir SolicitudesWeb</a></p>' +
    '<p>A solicitude queda co estado ' +
    '<strong>Nova</strong> para a súa xestión en AppSheet.</p>';
...
function actualizarCampoSolicitude_(
  folla,
  cabeceiras,
  fila,
  nomeCampo,
